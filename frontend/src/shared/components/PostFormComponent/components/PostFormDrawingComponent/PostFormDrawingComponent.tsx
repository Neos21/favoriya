...
export const PostFormDrawingComponent: FC<Props> = ({ setFormData, reloadTrigger }) => {
  ...
  return <>
    ...
    <Box component="div" ref={containerRef} sx={{ mt: 1, minWidth: '300px', minHeight: '300px', maxWidth: '600px', maxHeight: '600px', border: '1px solid', borderColor: 'grey.500' }}>
      <canvas ref={canvasRef} style={{ display: 'block', width: '100%', height: '100%' }}
        ...
        onTouchStart={event => { const { x, y } = getPosition(event); startDrawing(x, y, event); }}
        onTouchMove={event => { const { x, y } = getPosition(event); draw(x, y, event); }}
